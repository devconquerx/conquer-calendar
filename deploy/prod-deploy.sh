...
EOF
  ...
  say "6/6 · Retirando el backend viejo y reciclando Celery…"
  sleep "$DRAIN_SECONDS"
  if [[ -n "$legacy" ]]; then
    docker stop -t 45 app-django-1 >/dev/null && docker rm app-django-1 >/dev/null
    ok "Contenedor legacy app-django-1 retirado."
  fi
  docker rm -f app-node-ssr-1 >/dev/null 2>&1 || true
