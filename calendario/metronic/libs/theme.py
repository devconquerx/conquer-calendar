...
class KTTheme:
        ...
        if scope in KTTheme.htmlAttributes:
            for key in KTTheme.htmlAttributes[scope]:
                attributes.append('{}="{}"'.format(
                    key, KTTheme.htmlAttributes[scope][key]))
    ...
    def getSvgIcon(path, classNames='svg-icon', folder='media/icons/'):
        ...
        svg = open('./conquercrm' + KTTheme.asset(folder + path)).read()
    ...
    def setLayout(view, context={}):
        ...
        module = '_templates.{}._bootstrap.{}'.format(
            settings.KT_THEME_DIR.replace('/', '.'), layout)
        ...
        if not util.find_spec(module) == None:
            ...
            module = '_templates.{}._bootstrap.default'.format(
                settings.KT_THEME_DIR.replace('/', '.'))
            KTBootstrap = KTTheme.importClass(module, 'KTBootstrapDefault')
            KTBootstrap.init(context)

        return '{}/{}'.format(settings.KT_THEME_DIR, view)
