...
def _tag_check(done_tag, failed_tag, task_name_fragment, short_description):
    """Factory de columnas tri-estado: ✅ done, ⚠️ failed (link a Sentry), ⏳ pendiente."""
    def column(self, obj):
...
class LeadAdmin(admin.ModelAdmin):
    list_display = (
        ...
        'col_ac', 'col_nb', 'col_crm', 'col_welcome',
        'created',
    )
    ...
    col_meta = _tag_check('meta_capi_done', 'meta_capi_failed', 'process_meta_capi', 'Meta')
    col_tiktok = _tag_check('tiktok_events_done', 'tiktok_events_failed', 'process_tiktok_events', 'TikTok')
    col_google = _tag_check('google_ads_done', 'google_ads_failed', 'process_google_ads', 'Google')
    col_respondio = _tag_check('respondio_done', 'respondio_failed', 'process_respondio', 'Respondio')
    col_ac = _tag_check('activecampaign_done', 'activecampaign_failed', 'process_activecampaign', 'AC')
    col_nb = _tag_check('neverbounce_done', 'neverbounce_failed', 'process_neverbounce', 'NB')
    col_crm = _tag_check('crm_done', 'crm_failed', 'process_crm_send', 'CRM')
    col_welcome = _tag_check('welcome_email_done', 'welcome_email_failed', 'process_welcome_email', 'Welcome')
